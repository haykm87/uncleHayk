...
export class CookieService implements ICookieService {
  ...
  public get(name: string): any {
    if (this.platformService.isBrowser) {
      ...
      try {
        return JSON.parse((this.req && this.req.cookies && this.req.cookies[name]) ? this.req.cookies[name] : '');
      } catch (err) {
        return (this.req && this.req.cookies && this.req.cookies[name]) ? this.req.cookies[name] : null
      }
    }
  }
